...
using SendCallback = std::function<void(uint16_t dst, PacketType type,
...
public:
    ...
    uint16_t get_base_seq() const { return base_seq_; }
    uint16_t get_next_seq() const { return next_seq_; }
    bool     sender_window_full() const;
    void     reset_sender();

    // Receiver API
    bool init_receiver(uint16_t total_fragments, size_t fragment_size,
                       size_t file_size);
    bool receive_fragment(uint16_t seq, const uint8_t *data, size_t len);
    bool is_complete() const;
    const uint8_t *get_reassembly_buffer() const { return reassembly_buf_; }
    size_t get_file_size() const { return file_size_; }
    void cleanup_receiver();

    // Set the callback for outbound packets
    void set_send_callback(SendCallback cb) { send_cb_ = cb; }

    // Destination for sender-side retransmits and receiver-side ACKs
    void set_peer_addr(uint16_t addr) { peer_addr_ = addr; }
