...
void TransferEngine::tick_mesh_arq()
{
        ...
        for (uint8_t i = 0; i < oow_retx_count_; i++)
        {
            ...
            if (is_parity)
            {
                ...
                for (uint8_t d = 0; d < FEC_GROUP_SIZE; d++)
                {
                    ...
                    {
                        ESP_LOGW(TAG,
                                 "OOW parity read returned 0: seq=%u group=%u data_idx=%u offset=%zu frag_len=%zu size=%zu",
                                 seq,
                                 group,
                                 di,
                                 off,
            ...
            if (got == 0)
            {
                ...
                ESP_LOGW(TAG,
                         ...
                         seq,
                         is_parity ? 0U : static_cast<unsigned>(static_cast<size_t>(group * FEC_GROUP_SIZE + idx_in_group) * transfer_.fragment_size),
                         is_parity ? 0U : static_cast<unsigned>((transfer_.size > static_cast<size_t>(group * FEC_GROUP_SIZE + idx_in_group) * transfer_.fragment_size)
                                                                  ? (((transfer_.size - static_cast<size_t>(group * FEC_GROUP_SIZE + idx_in_group) * transfer_.fragment_size) < transfer_.fragment_size)
                                                                         ? (transfer_.size - static_cast<size_t>(group * FEC_GROUP_SIZE + idx_in_group) * transfer_.fragment_size)
                                                                         : transfer_.fragment_size)
                                                                  : 0),
                         transfer_.size);
                continue;
    ...
    if (redist_count_ > 0)
    {
        ...
        for (uint8_t r = 0; r < redist_count_; r++)
        {
            ...
            if (arq_[target].can_send_sequence(seq))
            {
                ...
                {
                    ESP_LOGW(TAG,
                             "Redistribution read returned 0: seq=%u target=%u data_idx=%u offset=%zu frag_len=%zu size=%zu",
                             seq,
                             target,
                             data_idx,
                             offset,
                ...
                int ret = arq_[target].send_fragment(seq, frag_buf_, got);
                if (ret < 0)
                {
                    redist_pending_[new_count++] = seq;
                }
           ...
           new_sent < MAX_NEW_FRAGS_PER_TICK)
    {
        ...
        else
        {
            ...
            {
                ESP_LOGW(TAG,
                         "New fragment read returned 0: seq=%u arq=%u data_idx=%u offset=%zu frag_len=%zu size=%zu",
                         seq,
                         arq_idx,
                         data_idx,
                         offset,
...
void TransferEngine::redistribute_dead_exit(uint8_t dead_idx)
{
    ...
    for (uint16_t p = 0; p < pending_count; p++)
    {
        ...
        if (arq_[target].can_send_sequence(seq))
        {
            ...
            {
                ESP_LOGW(TAG,
                         "Dead-exit redistribution read returned 0: seq=%u target=%u data_idx=%u offset=%zu frag_len=%zu size=%zu",
                         seq,
                         target,
                         data_idx,
                         offset,
