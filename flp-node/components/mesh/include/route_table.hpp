...
public:
                         ...
                         uint8_t hops_to_inet = 0xFF)
    {
        ...
        if (count_ < MAX_NEIGHBORS) {
            neighbors_[count_] = {addr, rssi, hops, hops_to_inet, now, ble, lora, false};
            count_++;
        } else {
            ...
            neighbors_[oldest_idx] = {addr, rssi, hops, hops_to_inet, now, ble, lora, false};
        }
    ...
    uint8_t min_hops_to_internet() const
    {
        ...
        uint8_t best = 0xFF;
        for (uint8_t i = 0; i < count_; i++) {
            if (neighbors_[i].has_internet || neighbors_[i].hops_to_internet < 0xFF) {
