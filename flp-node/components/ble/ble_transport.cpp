...
void BleTransport::init()
{
    ...
    initialized_ = true;
    ESP_LOGI(TAG,
             "BLE transport initialized, node_addr=0x%04X name=%s",
             node_addr_,
             name);
}
...
int BleTransport::on_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    switch (event->type)
    {
        case BLE_GAP_EVENT_CONNECT:
            if (event->connect.status == 0)
            {
                ...
                for (int i = 0; i < BLE_MAX_CONNECTIONS; i++)
                {
                    if (!self->peers_[i].connected)
                    {
                        ...
                        memcpy(self->peers_[i].peer_addr,
                               desc.peer_id_addr.val,
                               6);
                        self->peers_[i].rssi = 0;
                        self->peers_[i].connected = true;
                        ESP_LOGI(
                            TAG, "Peer connected, handle=%d slot=%d", ch, i);
                        break;
                    }
                }
            ...
            {
                ESP_LOGW(
                    TAG, "Connection failed, status=%d", event->connect.status);
            }
        ...
        case BLE_GAP_EVENT_DISCONNECT:
            ESP_LOGI(TAG,
                     "Peer disconnected, handle=%d reason=%d",
                     event->disconnect.conn.conn_handle,
                     event->disconnect.reason);
            for (int i = 0; i < BLE_MAX_CONNECTIONS; i++)
            {
                if (self->peers_[i].conn_handle ==
                    event->disconnect.conn.conn_handle)
                {
        ...
        case BLE_GAP_EVENT_DISC:
        {
            ...
            int rc = ble_hs_adv_parse_fields(
                &parsed, event->disc.data, event->disc.length_data);
            if (rc == 0)
            {
                fields = &parsed;
            }
            ...
            if (fields && fields->num_uuids128 > 0)
            {
                for (int i = 0; i < fields->num_uuids128; i++)
                {
                    ...
                    {
                        ESP_LOGI(
                            TAG, "Found FLP peer, RSSI=%d", event->disc.rssi);
...
int BleTransport::send(uint16_t peer_addr, const uint8_t *data, size_t len)
{
    ...
    for (int i = 0; i < BLE_MAX_CONNECTIONS; i++)
    {
        if (peers_[i].connected &&
            addr_from_ble(peers_[i].peer_addr) == peer_addr)
        {
            ...
            int rc = ble_gatts_notify_custom(
                peers_[i].conn_handle, tx_chr_val_handle_, om);
            if (rc != 0)
            {
