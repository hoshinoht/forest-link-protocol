...
void UartIngest::init(uart_port_t port, int tx_pin, int rx_pin, MeshManager *mgr)
{
    port_ = port;
    mgr_  = mgr;
...
void UartIngest::run()
{
    ...
    while (true) {
        ...
        case ParseState::SYNC1:
            if (byte == UART_SYNC1) state_ = ParseState::SYNC2;
            break;

        case ParseState::SYNC2:
            state_ = (byte == UART_SYNC2) ? ParseState::CMD : ParseState::SYNC1;
            break;
...
{
    if (len < 5) { // 4 bytes size + at least 1 byte filename
    ...
    if (name_len >= sizeof(filename_)) name_len = sizeof(filename_) - 1;
    memcpy(filename_, &payload[4], name_len);
    filename_[name_len] = '\0';

    ingest_buf_ = (uint8_t *)heap_caps_malloc(file_size, MALLOC_CAP_SPIRAM);
    if (!ingest_buf_) {
        ESP_LOGE(TAG, "PSRAM alloc failed for %lu bytes", (unsigned long)file_size);
        send_nack(UART_CMD_FILE_BEGIN, UART_ERR_ALLOC_FAIL);
        return;
    }
    ...
    ESP_LOGI(TAG, "FILE_BEGIN: \"%s\" (%lu bytes)", filename_, (unsigned long)file_size);
...
{
    uint8_t resp[5];
    resp[0] = mgr_->get_events()
              ? (xEventGroupGetBits(mgr_->get_events()) & FLP_EVT_WIFI_CONNECTED) ? 1 : 0
              : 0; // has_inet
