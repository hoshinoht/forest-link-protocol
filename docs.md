...
Feedback Question (Feb 1, 2026): How does MQTT work with ESP-NOW and LoRa?

MQTT works together with ESP-NOW and LoRa on separate layers.


Internal Mesh (ESP-NOW/LoRa): Nodes within the internal mesh communicate with one another using ESP-NOW or LoRa.
...
In our problem statement, nodes are deployed in a dense forest environment, which is not ideal for WiFi signals to communicate, and not every node is connected to the internet or WiFi network. Hence, protocols such as ESP-NOW and LoRa will be used for communications in the dense forest mesh environment. Only a select few nodes with WiFi will send the data out of the forest mesh to the cloud backend.
...
The Power and Range Trade-off: Existing solutions do not implement adaptive switching between ESP-NOW (high-throughput short-range) and LoRa (longer-range lower-throughput) based on network conditions, forcing them to only use one protocol with relatively low efficiency.

FLP v3.7 fills this gap by using an Adaptive Multi-Protocol Architecture that intelligently switches between ESP-NOW and LoRa for internal mesh communications based on power budget, hop count, and RSSI, while reserving Wi-Fi for MQTT-based external data exfiltration when connected to the internet.
...
Adaptive Protocol Selection: Developing an intelligent system that can dynamically choose between ESP-NOW and LoRa based on real-time power budget, hop counts, and RSSI measurements
...
Functional Adaptive Mesh: A stable mesh that dynamically selects between ESP-NOW or LoRa for inter-node communications, ensuring optimal energy efficiency and reliability
...
The ESP32-S3 nodes are connected using ESP-NOW for short, high-bandwidth range communications, and LoRa for longer-range communication, with access to the cloud MQTT backend being opportunistic, i.e. if a node can connect to the backend via WiFi, it will do so and keep note of that state in memory.

The ESP32 nodes are designed to be opportunistic, where they will occasionally attempt to connect to the cloud MQTT backend to check if they have access, and if they do, they will update their own state to reflect it.
...
All ESP32 nodes run an MQTT client and store a topic lookup table that is synchronised with the MQTT Admin (cloud backend) to ensure publishes are sent to the correct nodes.
Data transfer on the meshes will transfer one file at a time, not multiple

Data Transfer (Forest -> Cloud)
When there is data that needs to be transmitted from an ESP32 node to the internet, it will advertise its intent by broadcasting LoRa packets to the other nodes around it, with the intent of locating and electing nodes with internet connection to the MQTT Admin cloud backend.



Once the nodes have been elected for data transfer, they become exit nodes, where they will forward data to the cloud MQTT backend over WiFi using MQTT. The source node will start to send fragmented data packets over ESP-NOW or LoRa, where the choice of transport is based on factors such as power budget, priority, and available connection quality. Intermediate nodes become relays to pass the data over to the exit nodes.


The MQTT-based cloud transfer path adopts a selective repeat sliding window algorithm for flow control and to handle missing packets. The MQTT Admin is in charge of
...
Through this, it is possible for the MQTT Admin to publish MQTT packets back down to ESP32 nodes, and they are capable of being relayed or broadcast to other nodes that may not have direct access to the cloud backend.

This allows for control of these forest nodes from the cloud, so long as there is at least one node that has access to the cloud MQTT backend.

Functional Requirements
[FR-MESH1] The ESP32 nodes must be able to communicate with each other using either ESP-NOW or LoRa connections
...
[FR-MESH3] The ESP32 nodes must be capable of interfacing with the cloud MQTT backend over WiFi using an MQTT client
[FR-MESH4] The ESP32 nodes must be capable of parsing intent packets and sending back ACK packets notifying the sender if they have access to the cloud MQTT backend
[FR-MESH5] The ESP32 nodes must be capable of receiving back MQTT published packets from either the cloud backend or forwarded from LoRa/ESP-NOW
...
[FR-MQTT4] The ESP32 Node’s MQTT client must be capable of supporting different Quality of Service (QoS) levels, to ensure network reliability
